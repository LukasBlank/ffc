<template>
  <v-dialog v-model="showDialog" max-width="400" persistent>
    <v-card color="#2e2e2e">
        ...
        <v-icon v-if="options.type" size="0.9em" color="indigo" class="mx-2">
          {{
            options.type === 'sponsored'
              ...
              : options.type === 'curated'
              ? 'mdi-check-decagram'
              : options.type === 'official'
              ? 'mdi-flash-circle'
              : null
          }}
        </v-icon>
      ...
      <v-card-text v-if="options.message" class="text-left">{{
        options.message
      }}</v-card-text>
      ...
      <v-list v-if="options.table">
        <v-list-item v-if="options.tableHead">
          <v-list-item-content class="font-weight-bold">{{
            options.tableHead.name
          }}</v-list-item-content>
          <v-list-item-content class="font-weight-bold">{{
            options.tableHead.value
          }}</v-list-item-content>
        </v-list-item>
...
export default class Dialog extends Vue {
  ...
  async close(btn?: CustomDialogOptionsButton) {
    this.showDialog = false
    if (btn && btn.callback) {
      await btn.callback()
    }
