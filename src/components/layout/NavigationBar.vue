<template>
  <div class="NavBar">
    ...
    <v-app-bar :clipped-left="primaryDrawer.clipped" app :class="colorAppBar">
      ...
      <v-app-bar-nav-icon
        v-else
        @click.stop="togglePrimaryDrawer"
      ></v-app-bar-nav-icon>
...
export default class NavigationBar extends NavigationBarProps {
  ...
  get isInLearning() {
    return this.$route.name === 'Learn' || this.$route.name === 'Group Learn'
  }
  get colorAppBar() {
  ...
  navigate(next: string): void {
    if (this.$route.name == 'Study Group' && next != this.$route.path)
      this.$eventHub.$emit(Event.DESELECT_ALL_DECKS)
    if (this.$route.path != next) this.$router.push(next)
  }
}
</script>
